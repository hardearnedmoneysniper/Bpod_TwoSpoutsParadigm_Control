...
function Lick_Single_Spout
    ...
    for currentTrial = 1:5000    % 5000次不用改，防止数量不够
        ...
        % Sound cue状态   30s声音刺激cue    Port1  一直通电，0~255可以调整通电电压。
        sma = AddState(sma, 'Name', 'CueOn', ...
            'Timer', cueDuration, ...
            'StateChangeConditions', {'Port1In', 'Lick', 'Tup', 'NoResponse'}, ...
            'OutputActions', {'HiFi1', ['P' 0], RewardPort, 255});

        % 水嘴Port1被选择，给水奖励0.1S并结束cue
        sma = AddState(sma, 'Name', 'Lick', ...
            'Timer', rewardTime, ...
            'StateChangeConditions', {'Tup', 'InterTrial'}, ...
            'OutputActions', {RewardPort, 255});
        ...
        subplot(1, 2, 1);
        scatter(1:currentTrial, choices, 'filled', 'MarkerFaceColor', [0.2, 0.6, 1]);
        ...
        lickCount = length(find(choices == 1));
        hBar = bar([0, 1], [noChoiceCount, lickCount],'FaceColor', [169/255, 169/255, 169/255]);

        % add label
        text(0, noChoiceCount, num2str(noChoiceCount), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
        text(1, lickCount, num2str(lickCount), 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
       
        xlabel('Choice (0=No Choice, 1=Lick)','FontSize',12);
